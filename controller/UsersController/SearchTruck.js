...
export const getMatchingPostTrucks = async (req, res) => {
    ...
    WHERE 
        t.trucks_status = 5 
        AND pt."postTrucks_status" != 6
        AND ($1::timestamp IS NULL OR pt."postTrucks_dateTime" >= $1) 
        AND ($2::text IS NULL OR pt."postTrucks_from" = $2)
        AND ($3::text IS NULL OR pt."postTrucks_to" = $3)
        AND ($4::integer IS NULL OR pt."postTrucks_capacity_id" = $4)
        AND ($5::integer IS NULL OR t.trucks_type_id = $5)
    ORDER BY 
        pt."postTrucks_dateTime" DESC
    LIMIT 
        $6 OFFSET $7;
`;
    ...
    try {
        ...
        if (error.code === "ECONNRESET") {
            return res.status(500).json({ message: "Database connection was reset. Please try again." });
        }
        if (error.code === "ECONNREFUSED") {
            return res.status(500).json({ message: "Database connection was refused. Check your database server." });
        }
